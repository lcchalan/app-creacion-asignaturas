...
La numeración de Unidad/tema/subtema es responsabilidad del sistema y nunca de la IA. La fuente es la jerarquía normalizada de `AcademicOfferingUnit`, `AcademicOfferingUnitContent` y `AcademicOfferingUnitSubcontent`.
...
Además del bloque `resource` que marca la posición visual del recurso dentro del contenido, el sistema reconstruye el guion aceptado en `content.resources`. Esto permite que EVA consuma directamente Bloom, tipo, complejidad, herramienta, pantallas/escenas y bibliografía sin interpretar las tablas visuales.
