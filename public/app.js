...
function markdownToHtml(markdown) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const image = line.match(/^!\[([^\]]*)\]\((data:image\/[^)]+|https?:\/\/[^)]+)\)$/);
    if (image) {
      closeList();
...
async function generateWeek(adjustmentInstructions = "") {
  ...
  try {
    ...
    output.innerHTML = markdownToHtml(payload.content);
    weekStates[currentWeek] = { ...weekState(currentWeek), status: "draft", draftContent: payload.content };
...
$("#bulk-user-form").onsubmit = async (event) => {
  ...
  try {
    ...
    form.reset(); await loadAdminDashboard();
  } catch (error) { showAdminMessage(error.message, true); }
};
$("#assignment-form").onsubmit = async (event) => {
  ...
  try {
    ...
    showAdminMessage("Asignación guardada correctamente."); form.reset(); await loadAdminDashboard();
  } catch (error) { showAdminMessage(error.message, true); }
};
...
$("#indicator-editor").onsubmit = async (event) => {
  ...
  try {
    ...
    showAdminMessage("Indicador guardado y puntuaciones redistribuidas."); form.classList.add("hidden"); await loadAdminDashboard();
  } catch (error) { showAdminMessage(error.message, true); }
};
...
$("#checklist-form").onsubmit = async (event) => {
  ...
  try {
    const response = await authRequest(`/api/admin/checklists/${form.dataset.reviewId}`, { method: "PATCH", body: JSON.stringify({ decision, generalObservation: form.elements.generalObservation.value, items }) });
...
$("#instruction-form").onsubmit = async (event) => {
  ...
  const form = event.currentTarget;
  const values = Object.fromEntries(new FormData(form));
  values.activate = form.elements.activate.checked;
  Object.assign(values, scopeValues(form));
  ...
  values.conflictResolution = form.querySelector("[data-impact-resolution]")?.value || "";
  try {
    await authRequest("/api/admin/instructions", { method: "POST", body: JSON.stringify(values) });
    showAdminMessage("Nueva versión de la especificación guardada."); form.reset(); resetImpact("SPECIFICATION"); await loadAdminDashboard();
  } catch (error) { showAdminMessage(error.message, true); }
};
...
$("#knowledge-form").onsubmit = async (event) => {
  ...
  try {
    ...
    showAdminMessage("Nueva versión del documento guardada."); form.reset(); resetImpact("DOCUMENT"); await loadAdminDashboard();
  } catch (error) { showAdminMessage(error.message, true); }
};
